...
class Photos(db.Model):
    """Encapsulates all photo-related methods and meta-data"""
    ...
    Capture_Date = db.Column('Capture_Date', db.Date) # Manual entry in upload
    Place = db.Column('Place', db.String(150)) # Manual entry - most granular location i.e. building/park
    City = db.Column('City', db.String(150)) # Manual entry - i.e. Sydney
    Region = db.Column('Region', db.String(6)) # Automatically generated by the reverse geo-lookup from GPS metadata.  i.e. State/Territory
    Country = db.Column('Country', db.String(2)) # Automatically generated by the reverse geo-lookup from GPS metadata
    ...
    def get_time(cls, operation=None):
        """Gets the time value from all the photos based on the operation i.e. min/max"""
        if operation == 'min':
    ...
    def photo_hash(filename):
        """Provides hash value of given file"""
    ...
    def get_geotagging(exif):
        """Extracts geolocation information from given EXIF data"""
        geotagging = {}
    ...
    def reverse_geocode(coordinates):
        """Perform reverse geolocation lookup of coordinate against MapBox API"""
    ...
    def rotate_photo(img):
        """Rotates the photo to the correct orientation"""
        try:
    ...
    def generate_thumbnail(cls, photo, width, height):
        """Generates a thumbnail of the photo, given its current height and width, to determine if it's landscape or portrait"""
        if width > height:
